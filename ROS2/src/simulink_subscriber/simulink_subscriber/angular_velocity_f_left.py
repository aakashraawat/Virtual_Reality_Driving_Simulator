...
class AngularVelocityNode(Node):
    ...
    def __init__(self):
        super().__init__('angular_velocity_f_left')
        self.subscription = self.create_subscription(Twist, 'angular_velocity_f_left', self.listener_callback, 10)

        self.publisher_ = self.create_publisher(WheelAngularVelocity, 'angular_velocity_f_left_unity', 10)
...
def main(args=None):
    ...
    angular_velocity_f_left = AngularVelocityNode()
    rclpy.spin(angular_velocity_f_left)
    angular_velocity_f_left.destroy_node()
    rclpy.shutdown()
